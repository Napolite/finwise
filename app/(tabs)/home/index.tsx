...
import QuickAnalysis from "@/screen-components/home-components/quickAnalysis";
...
const Home = () => {
  ...
  const [showQAnalysis, setShowQAnalysis] = useState(false);
    ...
    <View className="bg-[#00D09E] min-h-full">
      {!showQAnalysis ? (
        <>
          ...
          <BalanceStatement />
          <HomeScreen
            handleScroll={handTableScroll}
            scrollY={scrollY}
            setShowQAnalysis={() => {
              setShowQAnalysis(true);
            }}
          />
        </>
      ) : (
